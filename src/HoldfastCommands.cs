...
{
    /// <summary>
    /// Game commands for the core Holdfast: Nations at War plugin
    /// </summary>
    public partial class HoldfastCore : CSPlugin
    {
        ...
        private void VersionCommand(IPlayer player, string command, string[] args)
        {
            if (player.IsServer)
            {
                ...
                $"Branch: {BuildInfo.Current.Scm.Branch}\nOxide.Holdfast Version: {HoldfastExtension.AssemblyVersion}");*/ // TODO: Implement server version
            }
            else
            {
                var format = Covalence.FormatText(lang.GetMessage("Version", this, player.Id));
                player.Reply(string.Format(format, HoldfastExtension.AssemblyVersion, Covalence.GameName, Server.Version, Server.Protocol));
            }
        }
